...
instance Monoid Hash where
  mempty = fst $ unsafePerformIO $ withHashPtrNew Mutable.unit
  mappend a b = fst $ unsafePerformIO $ withHashPtrNew (unsafeUseAsPtr2 a b . Mutable.concat)
...
infixl 7 <+
(<+) :: Hash -> ByteString -> Hash
(<+) h s = fst $ unsafePerformIO $ withHashPtrCopy h $ Mutable.append s

-- | /O(n)/ Prepend the hash of the 'ByteString' to the existing 'Hash'.
-- A significantly faster equivalent of @(('<>') . 'hash')@.
infixr 7 +>
(+>) :: ByteString -> Hash -> Hash
(+>) s h = fst $ unsafePerformIO $ withHashPtrCopy h $ Mutable.prepend s
...
(<|) h ss = fst $ unsafePerformIO $ withHashPtrCopy h $ Mutable.foldAppend ss
...
(|>) ss h = fst $ unsafePerformIO $ withHashPtrCopy h $ Mutable.foldPrepend ss

-- | /O(1)/ Parse the representation generated by 'show'.
parse :: String -> Maybe Hash
parse s = (\(h, r) -> h <$ r) $ unsafePerformIO $ withHashPtrNew $ Mutable.unserialize s
